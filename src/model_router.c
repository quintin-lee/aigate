...
static void free_route_cb(void *val)
{
  model_rec_t *m = val;
  free(m);
}
...
static int resolve_key(model_router_t *mr, const model_rec_t *rec,
                       model_rec_t *out)
{
  ...
  if (strncmp(rec->upstream_key_ref, "env:", 4) == 0) {
    ...
    if (env == NULL || env[0] == '\0') {
      AIGATE_LOG_ERROR("env key %s missing for model %s",
                       rec->upstream_key_ref + 4, rec->name);
      return -1;
    }
  ...
  if (strncmp(rec->upstream_key_ref, "pg:", 3) == 0) {
    ...
    if (secret_decrypt(mr->master, rec->upstream_key_ref + 3,
                       out->upstream_key, sizeof out->upstream_key, NULL) != 0) {
...
int model_router_resolve(model_router_t *mr, const char *model,
                         model_rec_t *out)
{
