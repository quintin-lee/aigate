...
static time_t utc_midnight_now(void)
{
  time_t now = time(NULL);
  return (time_t)(now - ((uint64_t)now % 86400));
}

static uint64_t acc_hash(long key_id, const char *model)
{
...
static int is_known_provider(const char *p)
{
  return p && (strcmp(p, "openai") == 0 || strcmp(p, "ollama") == 0 ||
               strcmp(p, "azure") == 0);
}

static um_prov_t *prov_slot(usage_meter_t *um, const char *p)
{
...
void um_record(usage_meter_t *um, long key_id, const char *model,
               int http_status, long prompt_tokens, long completion_tokens,
               uint64_t latency_ns, const char *provider)
{
  ...
  for (int i = 0; i < UM_ACC_CAP; i++) {
    ...
    if (a->in_use && a->key_id == key_id &&
        strcmp(a->model, model) == 0) {
      if (a->day != day)
        a->day = day;
      a->requests++;
  ...
  *n_out = 0;
  ...
  for (int i = 0; i < UM_ACC_CAP; i++) {
    ...
    if (a->in_use) {
      ...
      snprintf(out[flushed].model_name, sizeof out[flushed].model_name, "%s",
               a->model);
  ...
  if (n > 0 && um->ps != NULL) {
    ...
    if (ops != NULL && ops->flush_usage != NULL &&
        ops->flush_usage(ops->ctx, out, n) != 0)
      return -1;
  }
  *n_out = n;
  return 0;
...
long um_provider_percentile_ns(usage_meter_t *um, const char *provider,
                               double percentile)
{
  pthread_mutex_lock(&um->mtx);
  long v = 0;
...
long um_provider_count_below_ns(usage_meter_t *um, const char *provider,
                                long le_ns)
{
  pthread_mutex_lock(&um->mtx);
  long v = 0;
