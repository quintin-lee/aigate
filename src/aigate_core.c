...
int aigate_core_init(aigate_core *ac, pg_store_t *ps, const uint8_t *master32,
...
int aigate_write_json(aigate_response_ctx *rc, int status, const char *body,
                      size_t len)
{
  if (!rc->headers_sent) {
    rc->status = status;
...
int aigate_write_error(aigate_response_ctx *rc, int http_status,
...
int aigate_handle_request(aigate_core *ac, aigate_request_ctx *rq,
                          aigate_response_ctx *rc)
{
  ...
  if (model[0] == '\0' || !key_allows_model(&krec, model)) {
    aigate_write_error(rc, PIPE_FORBIDDEN, "auth_error",
  ...
  if (!provider_openai_supports(route.provider)) {
    aigate_write_error(rc, PIPE_UNSUPPORTED, "unsupported_provider",
  ...
  if (urc != 0 || status >= 500) {
    ...
    aigate_write_error(rc, PIPE_UPSTREAM, "upstream_error",
                       "upstream request failed");
    /* still meter the failure */
    um_record(ac->um, krec.key_id, model, PIPE_UPSTREAM, 0, 0, lat,
              route.provider);
    free(ubody);
    json_decref(jbody);
    key_rec_free(&krec);
