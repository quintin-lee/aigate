...
static const char SCRIPT_CIRCUIT_BREAKER_SYNC[] =
    ...
    "local d = redis.call('HMGET', key, 'state', 'fails', 'open_until')\n"
    ...
    "            state = 1\n"
    "            redis.call('HMSET', key, 'state', 1, 'fails', fails)\n"
    "            return {1, 1}\n"
    "        else\n"
    "            return {0, 2}\n"
    "        end\n"
    ...
    "    if state == 1 or fails > 0 then\n"
    "        redis.call('HMSET', key, 'state', 0, 'fails', 0, 'open_until', 0)\n"
    "    end\n"
    "    return {1, 0}\n"
    ...
    "        redis.call('HMSET', key, 'state', 2, 'open_until', open_until)\n"
    "        return {0, 2}\n"
    "    else\n"
    "        fails = fails + 1\n"
    "        if fails >= max_fails then\n"
