...
int metrics_render(usage_meter_t *um, char *out, size_t cap)
{
  ...
  n = snprintf(w, rem, "# HELP aigate_requests_total Total gateway requests.\n"
               ...
               um_total_requests(um), um_total_errors(um),
               um_total_tokens(um));
  if (n < 0 || (size_t)n >= rem)
    return -1;
  w += n;
  ...
  for (int i = 0; i < nprov; i++) {
    ...
    for (int b = 0; b < NUM_BUCKETS; b++) {
      ...
      n = snprintf(w, rem,
                   ...
                   p, BUCKET_LE[b], um_provider_count_below_ns(um, p, le_ns));
      if (n < 0 || (size_t)n >= rem)
        return -1;
      w += n;
      rem -= (size_t)n;
    ...
    n = snprintf(w, rem,
                 ...
                 p, (long)sum_ns, p, (long)sum_ns, p, count);
    if (n < 0 || (size_t)n >= rem)
      return -1;
  }
  *w = '\0';
