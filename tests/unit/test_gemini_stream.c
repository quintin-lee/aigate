...
TEST_CASE(test_gemini_streaming_bridge_chunks)
{
    ...
    const char* chunk3 =
        "data: {\"candidates\": [{\"content\": {\"parts\": [{\"text\": \" streaming!\"}], \"role\": \"model\"}, \"finishReason\": \"STOP\"}], \"usageMetadata\": {\"promptTokenCount\": 25, \"candidatesTokenCount\": 10}}\n\n";

    TEST_ASSERT(g_provider_gemini.stream_bridge_feed(b, chunk1, strlen(chunk1)) == 0, "feed chunk 1");
    TEST_ASSERT(g_provider_gemini.stream_bridge_headers_sent(b), "headers sent after chunk 1");
    TEST_ASSERT(g_provider_gemini.stream_bridge_feed(b, chunk2, strlen(chunk2)) == 0, "feed chunk 2");
    TEST_ASSERT(g_provider_gemini.stream_bridge_feed(b, chunk3, strlen(chunk3)) == 0, "feed chunk 3");
    ...
    TEST_ASSERT(strstr(sink.written, "\"delta\":{\"content\":\" Gemini\"}") != NULL, "delta Gemini");
    TEST_ASSERT(strstr(sink.written, "\"delta\":{\"content\":\" streaming!\"}") != NULL, "delta streaming");
...
TEST_CASE(test_gemini_streaming_fragmented_tcp)
{
    ...
    const char* full_payload =
        "data: {\"candidates\": [{\"content\": {\"parts\": [{\"text\": \"Frag\"}], \"role\": \"model\"}}]}\n\n"
        "data: {\"candidates\": [{\"content\": {\"parts\": [{\"text\": \"mented\"}], \"role\": \"model\"}, \"finishReason\": \"STOP\"}], \"usageMetadata\": {\"promptTokenCount\": 12, \"candidatesTokenCount\": 4}}\n\n";
