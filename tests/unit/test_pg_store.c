...
static int deep_copy_allowlist(key_rec_t *dst, const key_rec_t *src)
{
...
static int fake_get_key_by_hash(void *ctx, const char *key_hash, key_rec_t *out)
{
...
static int fake_list_models(void *ctx, model_rec_t *out, int cap, int *n)
{
  struct fake_db *db = ctx;
...
static int fake_get_model(void *ctx, const char *name, model_rec_t *out)
{
  struct fake_db *db = ctx;
...
static int fake_create_key(void *ctx, const key_rec_t *k, long *out_key_id)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < FAKE_CAP; i++) {
    struct fake_key *fk = &db->keys[i];
...
static int fake_update_key(void *ctx, const key_rec_t *k, int mask)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < FAKE_CAP; i++) {
    struct fake_key *fk = &db->keys[i];
...
static int fake_revoke_key(void *ctx, long key_id)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < FAKE_CAP; i++) {
    struct fake_key *fk = &db->keys[i];
...
static int fake_create_model(void *ctx, const model_rec_t *m)
{
  struct fake_db *db = ctx;
...
static int fake_update_model(void *ctx, const model_rec_t *m, int mask)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < db->n_models; i++) {
    ...
    if (mask & MMASK_ENDPOINT)
      snprintf(db->models[i].endpoint, sizeof db->models[i].endpoint, "%s",
               m->endpoint);
    if (mask & MMASK_PARAMS)
      snprintf(db->models[i].default_params_json,
               sizeof db->models[i].default_params_json, "%s",
               m->default_params_json);
    if (mask & MMASK_KEYREF)
      snprintf(db->models[i].upstream_key_ref,
               sizeof db->models[i].upstream_key_ref, "%s", m->upstream_key_ref);
...
static int fake_delete_model(void *ctx, const char *name)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < db->n_models; i++) {
    if (strcmp(db->models[i].name, name) == 0) {
...
static int fake_flush_usage(void *ctx, const usage_row_t *rows, int n)
{
  struct fake_db *db = ctx;
  for (int i = 0; i < n; i++) {
    int found = 0;
    for (int j = 0; j < db->n_usage; j++) {
      ...
      if (r->key_id == rows[i].key_id &&
          strcmp(r->model_name, rows[i].model_name) == 0 &&
          r->day == rows[i].day) {
...
static int fake_query_usage(void *ctx, long key_id, const char *model,
                            time_t from, time_t to, usage_row_t *out, int cap,
                            int *n)
{
  struct fake_db *db = ctx;
  *n = 0;
...
static void build_fake_ops(struct fake_db *db, pg_ops_t *ops)
{
...
TEST_CASE(test_pg_fake_key_lifecycle)
{
  ...
  key_rec_t out;
  TEST_ASSERT(pg_store_ops(ps)->get_key_by_hash(&db, "ab11", &out) == 0,
              "get by hash");
  TEST_ASSERT(out.key_id == 1 && out.rate_qps == 5 && out.n_allowed == 2,
              "round-trip fields");
  ...
  k.key_id = id;
  k.rate_qps = 9;
  TEST_ASSERT(pg_store_ops(ps)->update_key(&db, &k, KMASK_RATE) == 0,
              "update rate");
...
TEST_CASE(test_pg_fake_model_lifecycle)
{
  ...
  int n = 0;
  TEST_ASSERT(pg_store_ops(ps)->list_models(&db, buf, FAKE_CAP, &n) == 0,
              "list models");
  TEST_ASSERT(n == 1, "one model");

  memset(&out, 0, sizeof out);
  TEST_ASSERT(pg_store_ops(ps)->get_model(&db, "mock-model", &out) == 0,
              "get model");
  TEST_ASSERT(strcmp(out.endpoint, "http://127.0.0.1:9999") == 0, "endpoint");

  strcpy(m.endpoint, "http://127.0.0.1:9998");
  TEST_ASSERT(pg_store_ops(ps)->update_model(&db, &m, MMASK_ENDPOINT) == 0,
              "update endpoint");
  memset(&out, 0, sizeof out);
  pg_store_ops(ps)->get_model(&db, "mock-model", &out);
  TEST_ASSERT(strcmp(out.endpoint, "http://127.0.0.1:9998") == 0,
              "endpoint changed");

  m.enabled = 0;
  TEST_ASSERT(pg_store_ops(ps)->update_model(&db, &m, MMASK_ENABLED) == 0,
              "disable model");
  TEST_ASSERT(pg_store_ops(ps)->delete_model(&db, "mock-model") == 0,
              "delete model");
  TEST_ASSERT(pg_store_ops(ps)->get_model(&db, "mock-model", &out) == -1,
              "gone after delete");
...
TEST_CASE(test_pg_fake_usage_flush_and_query)
{
  ...
  TEST_ASSERT(pg_store_ops(ps)->query_usage(&db, 7, NULL, 1699900000, 1700100000,
                                            buf, 4, &n) == 0,
              "query all models");
  ...
  n = 0;
  TEST_ASSERT(pg_store_ops(ps)->query_usage(&db, 8, NULL, 1699900000, 1700100000,
                                            buf, 4, &n) == 0,
              "query other key");
