...
static int akg_get_key(void *ctx, const char *key_hash, key_rec_t *out)
{
...
static int akg_other(void *ctx, ...)
{
  (void)ctx;
  return -1;
}

static pg_ops_t build_akg_ops(struct akg_db *db)
{
  pg_ops_t ops;
...
static pg_store_t *open_akg_store(struct akg_db *db)
{
...
TEST_CASE(test_auth_key_resolve_normal)
{
  ...
  key_rec_t out;
  TEST_ASSERT(auth_key_resolve(&akc, "plaintext-secret", &out) == 0,
              "resolve known key");
  ...
  key_rec_t out2;
  TEST_ASSERT(auth_key_resolve(&akc, "plaintext-secret", &out2) == 0,
              "second resolve");
