...
TEST_CASE(test_secret_roundtrip)
{
    ...
    TEST_ASSERT(secret_encrypt(master, msg, strlen(msg), blob, sizeof blob) == 0, "encrypt ok");
    TEST_ASSERT(strncmp(blob, "76313a", 6) == 0, "hex starts with 'v1:'");
    TEST_ASSERT(secret_decrypt(master, blob, plain, sizeof plain, &out_len) == 0, "decrypt ok");
...
TEST_CASE(test_secret_tamper_and_wrong_key)
{
    ...
    TEST_ASSERT(secret_encrypt(master, "data", 4, blob, sizeof blob) == 0, "encrypt");
...
TEST_CASE(test_secret_hex_to_bytes)
{
    uint8_t b[32];
    TEST_ASSERT(
        hex_to_bytes32("00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff", b) == 0,
        "hex_to_bytes32 ok");
