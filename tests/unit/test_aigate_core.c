...
static int fdeep_allowlist(key_rec_t *dst, const key_rec_t *src)
{
...
static int fget_key(void *ctx, const char *key_hash, key_rec_t *out)
{
  struct fdb *db = ctx;
...
static int fget_model(void *ctx, const char *name, model_rec_t *out)
{
  struct fdb *db = ctx;
...
static int f_flush_rows(void *ctx, const usage_row_t *rows, int n)
{
  struct fdb *db = ctx;
  db->flush_calls++;
...
static void fbuild_ops(struct fdb *db, pg_ops_t *ops)
{
...
static void fkey_add(struct fdb *db, int slot, long key_id, const char *bearer,
                     int qps, long quota, const char *allowlist)
{
...
static int cap_set_header(void *impl, const char *name, const char *value)
{
  struct cap *c = impl;
  snprintf(c->hdrs + strlen(c->hdrs), sizeof c->hdrs - strlen(c->hdrs),
           "%s: %s\n", name, value);
  return 0;
}

static int cap_write(void *impl, const void *buf, size_t len, bool fin)
{
  struct cap *c = impl;
  (void)fin;
...
static aigate_response_ctx cap_rc(struct cap *c)
{
...
static int cap_has_header(const struct cap *c, const char *needle)
{
  return strstr(c->hdrs, needle) != NULL;
}

/** @brief Run one chat request through the pipeline. */
static const char *run(aigate_core *ac, const char *bearer, const char *model,
                       struct cap *out)
{
...
static int find_row(const usage_row_t *rows, int n, long key_id,
                   const char *model)
{
  for (int i = 0; i < n; i++)
    if (rows[i].key_id == key_id &&
        strcmp(rows[i].model_name, model) == 0)
      return i;
  return -1;
}
...
TEST_CASE(test_core_pipeline)
{
  ...
  snprintf(db.models[0].endpoint, sizeof db.models[0].endpoint, "%s",
           mock_upstream_base(mu));
  ...
  TEST_ASSERT(strstr(b1, "\"prompt_tokens\":7") != NULL,
              "upstream usage passed through: %.80s", b1);
  ...
  TEST_ASSERT(cap_has_header(&c6, "X-Upstream-Provider: openai"),
              "X-Upstream-Provider header");
  ...
  if (ig >= 0) {
    TEST_ASSERT(rows[ig].requests == 2, "2 requests, got %ld",
                rows[ig].requests);
    TEST_ASSERT(rows[ig].errors == 1, "1 error, got %ld", rows[ig].errors);
    TEST_ASSERT(rows[ig].prompt_tokens == 7, "prompt 7, got %ld",
                rows[ig].prompt_tokens);
    TEST_ASSERT(rows[ig].completion_tokens == 11, "completion 11, got %ld",
                rows[ig].completion_tokens);
  }
  /* rate-key: 2 successes merged (14/22 tokens) */
  int ir = find_row(rows, n, 2, "gpt-4o");
  TEST_ASSERT(ir >= 0, "rate-key gpt-4o row");
  if (ir >= 0) {
    TEST_ASSERT(rows[ir].requests == 2, "2 requests, got %ld",
                rows[ir].requests);
    TEST_ASSERT(rows[ir].prompt_tokens == 14, "prompt 14, got %ld",
                rows[ir].prompt_tokens);
    TEST_ASSERT(rows[ir].completion_tokens == 22, "completion 22, got %ld",
                rows[ir].completion_tokens);
...
TEST_CASE(test_provider_azure_build)
{
  ...
  snprintf(m.default_params_json, sizeof m.default_params_json, "%s",
           "{\"api-version\":\"2024-06-01\",\"temperature\":0.5}");

  char url[1024];
  char *merged = NULL;
  size_t mlen = 0;
  int rc = provider_openai_build(&m, "/chat/completions",
                                 "{\"model\":\"gpt-4o\"}", url, sizeof url,
                                 &merged, &mlen);
  ...
  TEST_ASSERT(
      ...
      "azure URL with api-version: %s", url);
  TEST_ASSERT(merged != NULL, "merged body");
  json_t *j = json_loads(merged, 0, NULL);
  TEST_ASSERT(j != NULL, "merged is JSON");
  TEST_ASSERT(json_object_get(j, "temperature") != NULL,
              "default temperature merged in");
  TEST_ASSERT(json_object_get(j, "model") != NULL, "request model kept");
  TEST_ASSERT(json_object_get(j, "api-version") == NULL,
              "api-version stripped from body");
  json_decref(j);
  free(merged);
}
...
TEST_CASE(test_provider_default_params_merge)
{
  ...
  size_t mlen = 0;
  int rc = provider_openai_build(&m, "/chat/completions",
                                 "{\"temperature\":1.0}", url, sizeof url,
                                 &merged, &mlen);
